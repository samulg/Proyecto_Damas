...
void Reglas::hacerReina() {
	...
	for (int i = 0; i < 20; i++) {
		if ((posicionSiguiente.y + 1 == listaFichasB[i]->posicion.y) && (posicionSiguiente.y == -9) && (posicionActual.y == posicionSiguiente.y + 1) && ((posicionActual.x == posicionSiguiente.x -1)|| (posicionActual.x == posicionSiguiente.x + 1))&& (posicionActual = listaFichasB[i]->posicion)){
			listaFichasB[i]->estado = 1;
			
		}
		else if ((posicionSiguiente.y + 2 == listaFichasB[i]->posicion.y) && (posicionSiguiente.y == -9)&&(posicionActual.y == posicionSiguiente.y + 2)&&((posicionActual.x == posicionSiguiente.x - 2) || (posicionActual.x == posicionSiguiente.x +2)) && (posicionActual = listaFichasB[i]->posicion)) {
			listaFichasB[i]->estado = 1;
...
bool Reglas::moverReina() {
	//la reina se puede mover todas las casillas que quiera en diagonal, para comer o para desplazarse
	//esto implica que la distancia en x y en y respecto de la posicionActual tienen que ser ó bien iguales ó bien iguales de signo opuesto
	int i;
	bool aux = false;
	int distx = 0;
	int disty = 0;
	distx = posicionSiguiente.x - posicionActual.x;
	disty = posicionSiguiente.y - posicionActual.y;
	
	//primero comprobamos que el movimiento es diagonal
